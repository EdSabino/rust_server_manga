...
pub struct FandomAdminGuard();

#[async_trait]
impl<'r, 'a> FromRequest<'r, 'a> for FandomAdminGuard {
    ...
    fn from_request(req: &'r Request<'_>) -> Outcome<Self, Self::Error> {
        ...
        match token.fandoms.iter().find(|item| item.admin && item.fandom_id == fandom_id) {
            Some(_) => Outcome::Success(FandomAdminGuard()),
